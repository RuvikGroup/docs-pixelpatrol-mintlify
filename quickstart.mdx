...
- A Pixel Patrol account ([Sign up here](https://pixelpatrol.app/auth))
...
<Steps>
  <Step title="Navigate to Sites">
    Go to the [Sites page](https://pixelpatrol.app/protected/sites) in your dashboard
  </Step>
  
  <Step title="Create a New Site">
    Click "Create Site" and fill in:
...
```bash cURL
curl -X POST https://pixelpatrol.app/api/media/submit \
...
const response = await fetch('https://pixelpatrol.app/api/media/submit', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://pixelpatrol.app/api/media/submit',
    headers={
        'Authorization': 'Bearer YOUR_API_KEY',
        'Content-Type': 'application/json'
    },
...
const checkStatus = async (mediaId) => {
  const response = await fetch(`https://pixelpatrol.app/api/media/${mediaId}/status`, {
    headers: {
      'Authorization': 'Bearer YOUR_API_KEY'
    }
  });
...
<CardGroup cols={2}>
  <Card title="Queue Page" icon="list" href="https://pixelpatrol.app/protected/queue">
    View all submitted media and their moderation status
  </Card>
  
  <Card title="Media Details" icon="magnifying-glass" href="https://pixelpatrol.app/protected/media-detail">
    Detailed view of individual media items with AI analysis
  </Card>
  
  <Card title="Site Analytics" icon="chart-line" href="https://pixelpatrol.app/protected/sites">
    Track moderation statistics and webhook performance
  </Card>
  
  <Card title="Usage Tracking" icon="gauge" href="https://pixelpatrol.app/protected/subscription">
...
- 📧 Email: [support@pixelpatrol.app](mailto:support@pixelpatrol.app)
