...
<Steps>
  <Step title="Navigate to Sites">
    Go to the [Sites page](https://app.pixelpatrol.net/protected/sites) in your dashboard
  </Step>
...
const checkStatus = async (mediaId) => {
  const response = await fetch('https://your-supabase-url.supabase.co/functions/v1/get-media-status', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
